...
public class CompileLogActivity extends BaseAppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getBooleanExtra("showingLastError", false)) {
            binding.topAppBar.setTitle("Æóðíàë ïîñëåäíåé êîìïèëÿöèè");
        } else {
            binding.topAppBar.setTitle("Æóðíàë êîìïèëÿöèè");
        }
        ...
        if (compileErrorSaver.logFileExists()) {
            binding.clearButton.setOnClickListener(v -> {
                if (compileErrorSaver.logFileExists()) {
                    ...
                    SketchwareUtil.toast("Æóðíàëû êîìïèëÿöèè áûëè î÷èùåíû.");
                } else {
                    SketchwareUtil.toast("Æóðíàëû êîìïèëÿöèè íå íàéäåíû.");
                }
        ...
        final String wrapTextLabel = "Ïåðåíîñ òåêñòà";
        final String monospacedFontLabel = "Ìîíîøèðèííûé øðèôò";
        final String fontSizeLabel = "Ðàçìåð øðèôòà";
